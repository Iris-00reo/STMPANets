...
def train(model):
    ...
    for epoch in range(args.epochs):
        model.train()
        for i, (x, y, mask, target_mask) in enumerate(tqdm(train_dataloader)):
            x, y, mask, target_mask = x.to(args.device), y.to(args.device), mask.to(args.device), target_mask.to(args.device)
            x = x * mask
            y = y * target_mask
        ...
        if loss < best_loss:
            ...
            torch.save(state, output_path + f"/{args.exp_name}_best.pth")
...
def evaluate(model, val_iter, scaler):
    ...
    k = 0
    with torch.no_grad():
        for i, (x, y, mask, target_mask) in enumerate(tqdm(val_iter)):
            x, y, mask, target_mask = x.to(args.device), y.to(args.device), mask.to(args.device), target_mask.to(args.device)
...
if __name__ == '__main__':
    args.exp_name = datetime.datetime.now().strftime('%Y%m%dT%H%M%S')
    print(args)
    log_path = './log_dir/' + args.dataset + '_miss' + str(args.mask_ratio) + '_' + args.exp_name + '.json'
